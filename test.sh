...
# BUILD
let "n = 0"
if [ -f j10/ex00/Makefile ]
then
	cp j10/ex00/Makefile Makefile
	...
	then 
		...
		if [ -f libft.a ]
		then
			errorExit "Error: failed to delete libft.a"
		fi
		make -s re || errorExit "Error: failed to make re"
		if [ -f libft.a ]
		then
			echo "make all, make clean, make fclean and make re: OK"
		else
			errorExit "Error: make re failed"
		fi
		make -s fclean
		let "n = n + 1"
	else 
		errorExit "Error: failed to create libft.a" 
	fi
	rm Makefile
fi
FILE="ft_foreach"
