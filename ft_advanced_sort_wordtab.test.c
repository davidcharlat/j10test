...
{
	{
		...
		int		ft_strcmp_inverse_for_test(char *s1, char *s2)
		{
			while (*s1 == *s2)
			{
				if (!*s1)
		...
		ft_advanced_sort_wordtab (split_test, &ft_strcmp_for_test_for_test);
		printf ("since *test = 'hello hello2 hi hi how are you ?',and function cmp = strcmp, words must be ordered: ? are hello hello2 hi hi how you\n");
		printf ("? ");
		...
		printf("OK\n");
		printf ("if function cmp returns the opposite value that strcmp would return, words'd now be ordered: you how hi hi hello2 hello are ?\n");
		ft_advanced_sort_wordtab (split_test, &ft_strcmp_inverse_for_test);
		printf ("you ");
		if (!ft_strcmp_for_test_for_test (split_test[0], "you"))
			printf ("OK\n");
		else
		{
			printf ("error: received %s", split_test[0]);
		...
		printf ("hello2 ");
		if (!ft_strcmp_for_test_for_test (split_test[4], "hello2"))
			printf ("OK\n");
		else
		{
			printf ("error: received %s", split_test[4]);
			return (free_split_test(split_test, 8) + 1);
		}
		printf ("? ");
		if (!ft_strcmp_for_test_for_test (split_test[7], "?"))
			printf ("OK\n");
		else
		{
			printf ("error: received %s", split_test[7]);
		...
		printf ("array still have to be terminated by zero\n");
		if (split_test[8] && split_test[8][0])
		{	
			printf ("error\n");
			return (1);
		}
		printf ("OK\nft_advanced_sort_wordtab OK\n");
